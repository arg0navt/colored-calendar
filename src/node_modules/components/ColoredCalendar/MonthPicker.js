...
export default class MonthPiker extends React.Component {
  ...
  setNumberMonth = (type) => {
    this.setState(prevState => {
      let monthDate = moment(prevState.month, this.context.format),
        active = type === "next" ? monthDate.add(1, "month").startOf("month").format(this.context.format) : monthDate.subtract(1, "month").startOf("month").format(this.context.format);
      if (this.props.skipMonth && (active === moment(this.props.skipMonth, this.context.format).startOf("month").format(this.context.format))) {
  ...
  setMonth = () => {
    this.setState({
      month: moment(this.props.date, this.context.format).add(this.state.number, "month").format(this.context.format),
    }, () => {
    ...
    return (
      <div className="Date-Month">
        <div className="Date-Month__control">
          ...
          ><i/></button>
          {moment(this.state.month, this.context.format).format("MMMM-YYYY")}
          <button
